...
static float* storeColorVals;

typedef struct{
  int t_id;
  int nthreads;
  float* storeColorVals;
  scene_t* scene;
} arg_t;
...
static void* getColor(void* arg){
    //THIS IS THE RENDERING PORTION
    /* for every pixel */
  arg_t* param = (arg_t*) arg;
  scene_t* scene = param->scene;
  int nthreads = param->nthreads;
  int t_id = param->t_id;
  
  for( int px=0 , place=0, rowSize = height*3; px<width; ++px )
    {
      if(px % nthreads != t_id){
	place += rowSize; //dealing with 3 columns
	continue;
      }
        const double x = pixel_dx * ((double)( px-(width/2) ));
        for( int py=0; py<height; ++py , place += 3 ) //this is to deal with 3 columns
        {
            ...
            for( int i=0; i<3; i++){
	      storeColorVals[place + i] = max( min(scaled_color[i], 255), 0);//pos + i 
	    }
...
main( int argc, char **argv )
{
    ...
    */
    ...
    storeColorVals = (float*) malloc(sizeof(float)*totalSize);
    pthread_t* thdArray = (pthread_t*) malloc(sizeof(pthread_t)*nthreads);
    arg_t* arguement = (arg_t*) malloc(sizeof(arg_t)*nthreads); //mult by nthreads
    //because we are using arguements to maintain information across the threads!
    if(thdArray == NULL || arguement == NULL || storeColorVals == NULL){
      fprintf(stderr , "Memory Allocation error");
      exit(1);
    }

    for(int j = 0; j < nthreads; j++){
      arguement[j].scene = &scene;
      arguement[j].nthreads = nthreads;
      arguement[j].t_id = j;
    }

    initConsts();

    //Below is essentially running getColor with thread(s)
    for(int t = 0; t < nthreads; t++){
      //below will run the command and if returns some false/null, will be error.
      //this is still executed tho!
      if(pthread_create(&thdArray[t], NULL, getColor, (void*) &arguement[t])){
	fprintf(stderr , "Error forming the threads");
	exit(1);
      }
    }

    //Below is rejoining the threads after completion
    for(int y = 0; y < nthreads; y++){
      if(pthread_join(thdArray[y], NULL)){
    ...
    int mainPlace = 0;
    int mainRowSize = height*3;
    while (mainPlace < totalSize){
      printf( "%.0f %.0f %.0f\n",
          storeColorVals[mainPlace], storeColorVals[mainPlace + 1], storeColorVals[mainPlace + 2] );
      mainPlace += 3;
      if(mainPlace % mainRowSize == 0){
	printf("\n");
      }
    }    
